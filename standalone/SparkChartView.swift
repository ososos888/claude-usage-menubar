...
// The x-axis is real time, with dotted vertical gridlines at round intervals.
final class SparkChartView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let tMin = points.first!.t, tMax = points.last!.t
        ...
        // dotted vertical time gridlines, aligned to "now" (rightmost sample)
        ...
        while true {
            ...
            // label: "now" / "-1h" / "-30m" below the baseline
            let ago = Double(i) * step
            let label: String = i == 0 ? "now" : (step >= 3600 ? "-\(Int(ago / 3600))h" : "-\(Int(ago / 60))m")
        ...
        area.line(to: NSPoint(x: x(tMax), y: plot.minY))
        area.line(to: NSPoint(x: x(tMin), y: plot.minY))
        area.close()
        ...
        // current-value dot
        let last = NSPoint(x: x(tMax), y: y(pcts[pcts.count - 1]))
